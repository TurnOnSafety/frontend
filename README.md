...
TurnOnSafety 프로젝트의 프론트엔드를 담당하는 레포입니다.

## 개발 방향

- 스택:

  프로젝트 특성상 SEO를 크게 신경 쓸 필요가 없어 CSR로 개발환경을 세팅했습니다. 데이터 시각화에서 사용자 인터랙션을 위해 CSS in JS 라이브러리를 사용하였습니다.

- 폴더구조:

  MVP를 만들고 그 후 기능 확장을 하는 방식이라, 어떤 기능이 필요할지 알 수 없습니다. 따라서 아토믹으로만 가져가기엔 코드 정리가 안될 듯 하여 FSD를 간단하게 도입하였습니다.

- 이슈 [많은 데이터를 처리하면서 사용자 경험 높이기]
  - 웹소켓을 사용하여 실시간 데이터를 받아오거나 긴급 호출을 해야하는 경우가 있어, 네트워크 비용에 대한 고민
  - 센서로 수집된 데이터는 수정 불가능한 불변성을 가진 데이터이므로, 프론트에서 데이터를 어떻게 관리할지?
    - 예시 상황: 데이터의 정렬 변경

## Stack

- React, TS+SWC, Vite 를 사용하여 개발환경 세팅
- 데이터 시각화 : D3(예정)
- 스타일 : Styled-components

## 폴더 구조(GPT 작성)

본 프로젝트는 MVP 개발과 이후 확장성을 고려하여 **Atomic Design**과 **Feature-Sliced Design (FSD)** 개념을 조합하였습니다.
...
- `/pages`  
  **라우팅 단위** 화면을 구성합니다.  
  (Next.js를 사용하는 경우, 라우터 설정이 자동 연동됩니다.)
